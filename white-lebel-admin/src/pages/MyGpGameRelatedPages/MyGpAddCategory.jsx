...
const MyGpAddCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/mygp-categories/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        toast.success("Category updated successfully");
      } else {
        await api.post("/api/mygp-categories", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/mygp-categories/${id}`);
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Create and manage MyGP game categories from your master admin
              panel.
            </p>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-black text-emerald-100">
                  Master Category Ready
                </p>
                <p className="text-xs text-emerald-200/80">
                  Manage once, use globally
                </p>
              </div>
            </div>
          </div>
      ...
      >
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-400">
                BN/EN content with category icon
              </p>
            </div>
        ...
        <div className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Category preview before save
          </p>

          <div className="mt-5 overflow-hidden rounded-[28px] border border-white/10 bg-black/30 p-6 text-center">
            <div className="mx-auto flex h-28 w-28 items-center justify-center overflow-hidden rounded-3xl border border-cyan-300/30 bg-black/40">
              {iconPreview ? (
                <img
                  src={iconPreview}
                  alt="Icon"
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">MyGP Categories</h2>
            <p className="text-sm text-slate-400">
        ...
        {listLoading ? (
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {categories.map((cat) => (
              <div
                key={cat._id}
                className="overflow-hidden rounded-[28px] border border-white/10 bg-black/25 p-5 shadow-xl transition hover:-translate-y-1 hover:border-cyan-300/30"
              >
                <div className="mx-auto flex h-24 w-24 items-center justify-center overflow-hidden rounded-3xl border border-cyan-300/30 bg-black/40">
...
export default MyGpAddCategory;
