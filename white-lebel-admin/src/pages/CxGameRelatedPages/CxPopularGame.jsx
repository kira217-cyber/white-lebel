...
const CxPopularGame = () => {
  ...
  const loadGames = async () => {
    try {
      ...
      const res = await api.get("/api/master/cx-popular-games", {
        params: {
          search,
          status: statusFilter,
          limit: 100,
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load CX popular games",
      );
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/master/cx-popular-games/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        toast.success("CX popular game updated successfully");
      } else {
        await api.post("/api/master/cx-popular-games", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        toast.success("CX popular game created successfully");
      }
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm(
      "Are you sure you want to delete this CX popular game?",
    );

    if (!ok) return;

    try {
      await api.delete(`/api/master/cx-popular-games/${id}`);

      toast.success("CX popular game deleted successfully");
      setGames((prev) => prev.filter((item) => item._id !== id));

      if (editing?._id === id) resetForm();
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Failed to delete CX popular game",
      );
    }
  };
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      await api.patch(
        `/api/master/cx-popular-games/${editing._id}/remove-image`,
      );
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Add CX popular game by Game ID, Bangla/English title, landscape
              image, order and status.
            </p>
          </div>
      ...
      >
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">
                {editing ? "Update CX Popular Game" : "Create CX Popular Game"}
              </h2>
              <p className="text-sm text-slate-400">
                Add Game ID, Bangla/English title and landscape image.
              </p>
            </div>
        ...
        <div className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
          ...
          <div className="mt-5 overflow-hidden rounded-3xl border border-cyan-300/20 bg-black/35 p-4">
            <div className="aspect-video w-full overflow-hidden rounded-2xl border border-cyan-300/30 bg-cyan-300/10">
              {preview ? (
                <img
                  src={preview}
                  alt="Popular Game"
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">CX Popular Game List</h2>
            <p className="text-sm text-slate-400">
        ...
        {listLoading ? (
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {games.map((game) => (
              ...
              >
                <div className="aspect-video w-full bg-cyan-300/10">
    ...
    <div>
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-cyan-300/30 bg-black/25 p-4 text-center transition hover:border-cyan-300/60 hover:bg-cyan-300/5">
        {preview ? (
          ...
          <div className="flex aspect-video w-full flex-col items-center justify-center rounded-2xl bg-black/30">
