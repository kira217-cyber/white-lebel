...
const BcSpotsGame = () => {
  ...
  const loadSports = async () => {
    try {
      ...
      const res = await api.get("/api/master/bc-sports", {
        params: {
          search,
      ...
      toast.error(error?.response?.data?.message || "Failed to load BetChokkor sports");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/master/bc-sports/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("BetChokkor sport updated successfully");
      } else {
        await api.post("/api/master/bc-sports", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("BetChokkor sport created successfully");
      }
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm("Are you sure you want to delete this BetChokkor sport?");
    if (!ok) return;

    try {
      await api.delete(`/api/master/bc-sports/${id}`);

      toast.success("BetChokkor sport deleted successfully");
      setSports((prev) => prev.filter((item) => item._id !== id));

      if (editing?._id === id) resetForm();
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Failed to delete BetChokkor sport",
      );
    }
  };
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Create, update and manage BetChokkor sport games with icon preview.
            </p>
          </div>
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">
                {editing ? "Update BetChokkor Sport" : "Create BetChokkor Sport"}
              </h2>
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">BetChokkor Sport List</h2>
            <p className="text-sm text-slate-400">
