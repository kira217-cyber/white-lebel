...
const initialUsedOrders = { categoryOrders: [], homeOrders: [] };

// "" / 0 both mean "no position set" — those providers always render after
// every numbered one, so they never take part in a duplicate check.
const toOrderNumber = (value) => {
  const num = Number(value);
  return Number.isFinite(num) && num > 0 ? Math.floor(num) : 0;
};

const findOrderOwner = (usedList, value, currentId) => {
  ...
  return (
    usedList.find(
      (item) =>
        Number(item?.order) === order && String(item?.id) !== String(currentId),
    ) || null
  );
};
...
const BcGameProvider = () => {
  ...
  // homeOrder clashes can come from any category, so the used numbers come
    ...
    try {
      ...
      setUsedOrders({
        categoryOrders: Array.isArray(data.categoryOrders)
          ? data.categoryOrders
          : [],
        homeOrders: Array.isArray(data.homeOrders) ? data.homeOrders : [],
      });
    } catch {
  ...
  useEffect(() => {
    loadUsedOrders(form.categoryId);
  ...
  const startEdit = (provider) => {
    ...
    setForm({
      ...
      order: provider?.order > 0 ? String(provider.order) : "",
      homeOrder: provider?.homeOrder > 0 ? String(provider.homeOrder) : "",
    });
  ...
  const orderOwner = useMemo(
    () => findOrderOwner(usedOrders.categoryOrders, form.order, editing?._id),
    [usedOrders.categoryOrders, form.order, editing],
  );

  const homeOrderOwner = useMemo(
    () => findOrderOwner(usedOrders.homeOrders, form.homeOrder, editing?._id),
    [usedOrders.homeOrders, form.homeOrder, editing],
  );

  const hasOrderConflict = Boolean(orderOwner || homeOrderOwner);
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasOrderConflict) {
      return toast.error("This order number is already added to another provider");
    }
    ...
    try {
      ...
      fd.append("order", String(toOrderNumber(form.order)));
      fd.append("homeOrder", String(toOrderNumber(form.homeOrder)));
      ...
      >
        <div className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <OrderField
              label="Order"
              hint={`Position in the provider row of ${
            ...
            <OrderField
              label="Home Order"
              hint="Position in the home page provider row. This number is shared across every category."
              value={form.homeOrder}
              conflict={homeOrderOwner}
              onChange={(value) => setForm({ ...form, homeOrder: value })}
              inputClass={inputClass}
              labelClass={labelClass}
            />
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            <button
              type="submit"
              disabled={loading || hasOrderConflict}
              ...
              >
                ...
                <div className="mt-5 text-center">
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    ...
                    {provider.order > 0 && (
                      <span className="rounded-xl bg-cyan-300/15 px-3 py-1 text-xs font-black text-cyan-200">
                        #{provider.order}
                      </span>
                    )}

                    {provider.homeOrder > 0 && (
                      <span className="rounded-xl bg-emerald-300/15 px-3 py-1 text-xs font-black text-emerald-200">
                        HOME #{provider.homeOrder}
                      </span>
                    )}
