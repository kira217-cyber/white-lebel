...
const initialUsedOrders = { categoryOrders: [], providerOrders: [] };

// "" / 0 both mean "no position set" — those games always render after
// every numbered game, so they never take part in a duplicate check.
const toOrderNumber = (value) => {
  const num = Number(value);
  return Number.isFinite(num) && num > 0 ? Math.floor(num) : 0;
};

const findOrderOwner = (usedList, value, currentGameId) => {
  const order = toOrderNumber(value);
  ...
  return (
    usedList.find(
      (item) =>
        ...
        String(item?.id) !== String(currentGameId),
    ) || null
  );
};
...
const BcGame = () => {
  ...
  const [usedOrders, setUsedOrders] = useState(initialUsedOrders);
  ...
  // categoryOrder clashes can live under a different provider of the same
  // category, so the used numbers come from the server rather than from the
  // provider-scoped selectedGames list already in state.
  const loadUsedOrders = async (
    ...
    try {
      ...
      setUsedOrders({
        categoryOrders: Array.isArray(data.categoryOrders)
          ? data.categoryOrders
          : [],
  ...
  useEffect(() => {
    loadUsedOrders(selectedCategoryId, selectedProviderDbId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedCategoryId, selectedProviderDbId]);
  ...
  const openEditModal = (selectedDoc, oracleGame = null) => {
    ...
    setEditForm({
      ...
      categoryOrder: selectedDoc.categoryOrder > 0
        ? String(selectedDoc.categoryOrder)
        : "",
      providerOrder: selectedDoc.providerOrder > 0
        ? String(selectedDoc.providerOrder)
        : "",
      ...flags,
    });
  ...
  const categoryOrderOwner = useMemo(
    () =>
      findOrderOwner(
        usedOrders.categoryOrders,
        editForm.categoryOrder,
        editingGame?._id,
      ),
    [usedOrders.categoryOrders, editForm.categoryOrder, editingGame],
  );

  const providerOrderOwner = useMemo(
    () =>
      findOrderOwner(
        usedOrders.providerOrders,
        editForm.providerOrder,
        editingGame?._id,
      ),
    [usedOrders.providerOrders, editForm.providerOrder, editingGame],
  );

  const hasOrderConflict = Boolean(categoryOrderOwner || providerOrderOwner);
  ...
  const handleUpdate = async (e) => {
    ...
    if (hasOrderConflict) {
      return toast.error("This order number is already added to another game");
    }
    ...
    try {
      ...
      fd.append("categoryOrder", String(toOrderNumber(editForm.categoryOrder)));
      fd.append("providerOrder", String(toOrderNumber(editForm.providerOrder)));
                    ...
                    {selected && (
                      <p className="truncate">
                        ...
                        <span className="font-black text-cyan-200">
                          {selectedDoc?.categoryOrder > 0
                            ? `#${selectedDoc.categoryOrder}`
                            : "—"}
                        </span>{" "}
                        · Provider{" "}
                        <span className="font-black text-emerald-200">
                          {selectedDoc?.providerOrder > 0
                            ? `#${selectedDoc.providerOrder}`
                            : "—"}
                        </span>
                      </p>
                    )}
            ...
            <form onSubmit={handleUpdate} className="space-y-5">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <OrderField
                  ...
                  hint={`Position on the ALL page of ${
                    selectedCategoryName || "this category"
                  }. Empty means no fixed position.`}
                  value={editForm.categoryOrder}
                  conflict={categoryOrderOwner}
                  onChange={(value) =>
                    setEditForm((prev) => ({ ...prev, categoryOrder: value }))
                  }
                ...
                <OrderField
                  ...
                  hint={`Position on the ${
                    selectedProviderCode || "provider"
                  } page. Empty means no fixed position.`}
                  value={editForm.providerOrder}
                  conflict={providerOrderOwner}
                  onChange={(value) =>
                    setEditForm((prev) => ({ ...prev, providerOrder: value }))
                  }
                  inputClass={inputClass}
                  labelClass={labelClass}
                />
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <button
                  type="submit"
                  disabled={hasOrderConflict}
                  className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-500 to-emerald-500 px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Save Changes
                </button>
