...
const BcFeaturedGame = () => {
  ...
  const loadGames = async () => {
    try {
      ...
      const res = await api.get("/api/master/bc-featured-games", {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load BetChokkor featured games",
      );
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/master/bc-featured-games/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("BetChokkor featured game updated successfully");
      } else {
        await api.post("/api/master/bc-featured-games", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("BetChokkor featured game created successfully");
      }
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm(
      "Are you sure you want to delete this BetChokkor featured game?",
    );
    if (!ok) return;

    try {
      await api.delete(`/api/master/bc-featured-games/${id}`);
      toast.success("BetChokkor featured game deleted successfully");
      setGames((prev) => prev.filter((item) => item._id !== id));
      if (editing?._id === id) resetForm();
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Failed to delete BetChokkor featured game",
      );
    }
  };
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      await api.patch(`/api/master/bc-featured-games/${editing._id}/remove-image`);
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Add BetChokkor featured game by Game ID, Bangla/English title, square image,
              order and status.
            </p>
          </div>
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">
                {editing ? "Update BetChokkor Featured Game" : "Create BetChokkor Featured Game"}
              </h2>
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">BetChokkor Featured Game List</h2>
            <p className="text-sm text-slate-400">
              Total {games.length} featured games found
            </p>
          </div>
        ...
        {listLoading ? (
          ...
          <div className="rounded-3xl border border-white/10 bg-black/30 p-10 text-center text-slate-400">
            No BetChokkor featured games found.
          </div>
        ) : (
