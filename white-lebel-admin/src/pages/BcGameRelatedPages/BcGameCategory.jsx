...
const BcGameCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/master/bc-game-categories/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Category updated successfully");
      } else {
        await api.post("/api/master/bc-game-categories", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/master/bc-game-categories/${id}`);
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Create, update and manage BetChokkor project game categories.
            </p>
          </div>
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">BetChokkor Game Categories</h2>
            <p className="text-sm text-slate-400">
