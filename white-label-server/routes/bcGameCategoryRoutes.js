...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const category = await BcGameCategory.create({
        categoryName: {
      ...
      return successResponse(
        res,
        "BetChokkor game category created successfully.",
...
router.get("/admin/all", protectMasterAdmin, async (req, res) => {
  try {
    ...
    const categories = await BcGameCategory.find(query).sort({
      order: 1,
      createdAt: -1,
    });
    ...
    return successResponse(
      res,
      "BetChokkor categories fetched successfully.",
...
router.get("/", async (req, res) => {
  try {
    const categories = await BcGameCategory.find({ status: "active" }).sort({
      order: 1,
      createdAt: -1,
    });
    ...
    return successResponse(
      res,
      "BetChokkor active categories fetched successfully.",
      categories.map((item) => formatCategory(req, item)),
    );
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
...
router.get("/:id", protectMasterAdmin, async (req, res) => {
  try {
    const category = await BcGameCategory.findById(req.params.id);

    if (!category) {
      return errorResponse(res, "BetChokkor category not found.", 404);
    }

    return successResponse(
      res,
      "BetChokkor category fetched successfully.",
      formatCategory(req, category),
    );
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      return successResponse(
        res,
        "BetChokkor category updated successfully.",
        formatCategory(req, category),
      );
    } catch (error) {
      if (req.file) deleteLocalFile(filePath(req.file));
...
router.delete("/:id", protectMasterAdmin, async (req, res) => {
  try {
    ...
    await BcGameCategory.findByIdAndDelete(category._id);

    if (oldImage) deleteLocalFile(oldImage);

    return successResponse(res, "BetChokkor category deleted successfully.", {
