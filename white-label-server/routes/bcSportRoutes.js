...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const sport = await BcSport.create({
        name: {
          bn: nameBn,
          en: nameEn,
        },
      ...
      return successResponse(
        res,
        "BetChokkor sport created successfully.",
  ...
  try {
    ...
    const [sports, total] = await Promise.all([
      BcSport.find(query)
        ...
        .skip(skip)
        .limit(limitNum),
      BcSport.countDocuments(query),
    ]);

    return successResponse(res, "BetChokkor sports fetched successfully.", {
...
router.get("/active/list", async (req, res) => {
  try {
    const sports = await BcSport.find({ isActive: true }).sort({
      order: 1,
      createdAt: -1,
    });
    ...
    return successResponse(
      res,
      "BetChokkor active sports fetched successfully.",
...
router.get("/:id", protectMasterAdmin, async (req, res) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return errorResponse(res, "Invalid BetChokkor sport id.", 400);
    }

    const sport = await BcSport.findById(req.params.id);

    if (!sport) {
      return errorResponse(res, "BetChokkor sport not found.", 404);
    }

    return successResponse(
      res,
      "BetChokkor sport fetched successfully.",
      formatSport(req, sport),
    );
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      return successResponse(
        res,
        "BetChokkor sport updated successfully.",
        formatSport(req, sport),
      );
    } catch (error) {
      if (req.file) deleteLocalFile(filePath(req.file));
...
router.delete("/:id", protectMasterAdmin, async (req, res) => {
  try {
    ...
    const sport = await BcSport.findByIdAndDelete(req.params.id);
    ...
    return successResponse(
      res,
      "BetChokkor sport deleted successfully.",
      formatSport(req, sport),
    );
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
