...
/* ======================================================
   ...
   GET /api/master/mygp-games/oracle/PG
====================================================== */
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const provider = await MasterMyGpGameProvider.findOne({
        _id: providerDbId,
        categoryId,
      });
      ...
      if (!provider) {
        return errorResponse(
          res,
          "MyGP provider not found under this category.",
          404,
        );
      }
      ...
      const exists = await MasterMyGpGame.findOne({
        providerDbId,
      ...
      const game = await MasterMyGpGame.create(payload);

      return successResponse(
        res,
        "MyGP game added successfully.",
        game,
        201,
      );
    } catch (error) {
...
router.get("/", protectMasterAdmin, async (req, res) => {
  try {
    ...
    return successResponse(res, "MyGP games fetched successfully.", {
      games,
      meta: {
        page: pageNum,
        limit: limitNum,
...
/* ======================================================
   GET SINGLE MYGP GAME
====================================================== */

router.get("/:id", protectMasterAdmin, async (req, res) => {
  try {
    const game = await MasterMyGpGame.findById(req.params.id)
      .populate("categoryId", "categoryName categoryTitle status")
      .populate("providerDbId", "providerName providerCode status");

    if (!game) {
      return errorResponse(res, "MyGP game not found.", 404);
    }

    return successResponse(res, "MyGP game fetched successfully.", game);
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (providerDbId !== undefined) {
        ...
        const provider = await MasterMyGpGameProvider.findById(providerDbId);

        if (!provider) {
          return errorResponse(res, "MyGP provider not found.", 404);
        }
      ...
      return successResponse(res, "MyGP game updated successfully.", game);
    } catch (error) {
      if (error?.code === 11000) {
        return errorResponse(
          res,
...
router.patch("/:id/remove-image", protectMasterAdmin, async (req, res) => {
  try {
    ...
    return successResponse(res, "MyGP game custom image removed.", game);
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
  }
});
...
router.delete("/:id", protectMasterAdmin, async (req, res) => {
  try {
    const game = await MasterMyGpGame.findByIdAndDelete(req.params.id);

    if (!game) {
      return errorResponse(res, "MyGP game not found.", 404);
    }

    return successResponse(res, "MyGP game deleted successfully.", game);
  } catch (error) {
    return errorResponse(res, error.message || "Server error", 500);
  }
});
